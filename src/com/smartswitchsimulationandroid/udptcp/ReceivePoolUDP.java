...
import com.smartswitchsimulationandroid.command.AppSearchWifi;
...
public class ReceivePoolUDP {
	...
	public synchronized void InsertSendItem(byte[] item, String APPIP) {
		if (sendQueue.size() < MAX_AVAILABLE) {
			SendBuffClass senditem = new SendBuffClass(item.length, APPIP);
			...
			while (!Thread.currentThread().isInterrupted() && !isOver) {
				try {
					...
					if (udata != null) {
						...
						case UDPTCPCommand.APP_SEARCH_WIFIMODULE:{
							Message msg = new Message();
							AppSearchWifi request = new AppSearchWifi(udata);

							Log.d("UDP", "localnetwork model" + Hex.encodeHexStr(udata.data));
							msg.what = ActivityHandlerParams.LocalNetWorkSearchDevice;
							msg.obj = udata.IP;
						...
						case UDPTCPCommand.APP_GET_DEVICE_STATE_UDP:{
							Message msg = new Message();
							AppGetDeviceStateUDP request = new AppGetDeviceStateUDP(udata);
...
//						}
	...
	private class ReciveThread extends Thread {
		...
		@Override
		public void run() {
			byte[] message = new byte[DeviceVariant.MessageLength];
			try {
				...
				while (!isStopped && !Thread.currentThread().isInterrupted()) {
					...
					if (s != null) {
						...
						DatagramPacket datagramPacket = new DatagramPacket(message, DeviceVariant.MessageLength);
